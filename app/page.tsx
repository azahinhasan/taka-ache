...
export default function Home() {
  ...
  const requestLocationAndFetchATMs = async () => {
    ...
    try {
      ...
      const atms = await fetchATMLocations(
        location.lat,
        location.lon,
        1000,
        googleApiKey,
      ...
      if (err instanceof GeolocationPositionError) {
        if (err.code === err.PERMISSION_DENIED) {
          ...
          setError(
            "Location unavailable. Using default location (Dhaka, Bangladesh).",
          );
          loadDefaultLocation();
        } else {
          setError(
            "Location request timeout. Using default location (Dhaka, Bangladesh).",
          );
          loadDefaultLocation();
        }
      } else {
  ...
  const handleCustomLocationSearch = async (
    ...
    try {
      ...
      setError(
        "Failed to load ATM locations at this location. Please try again.",
      );
    } finally {
      setIsLoading(false);
    }
